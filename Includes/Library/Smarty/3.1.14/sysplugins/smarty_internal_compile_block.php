...
class Smarty_Internal_Compile_Block extends Smarty_Internal_CompileBase
{
    ...
    public function compile($args, $compiler)
    {
        ...
        $save  = array(
            ...
            $compiler->nocache,
            $compiler->smarty->merge_compiled_includes,
    ...
    public static function saveBlockData($block_content, $block_tag, $template, $filepath)
    {
        ...
        if (0 == preg_match(
                "!({$_ldl}{$al}block\s+)(name=)?(\w+|'.*'|\".*\")(\s*?)?((append|prepend|nocache)?(\s*)?(hide)?)?(\s*{$_rdl})!",
                $block_tag,
                $_match
            )
        ) {
            ...
            if ($_match[8] != 'hide' || isset($template->block_data[$_name])) { // replace {$smarty.block.child}
                ...
                if (0 != preg_match_all(
                        "!({$_ldl}{$al}block\s+)(name=)?(\w+|'.*'|\".*\")([\s\S]*?)(hide)?(\s*{$_rdl})([\s\S]*?)(.*)?({$_ldl}{$al}/block\s*{$_rdl})!",
                        $block_content,
                        $_match2
                    )
                ) {
                        ...
                        if ($_match2[5][$key] != 'hide' || isset($template->block_data[$_name2])) {
                            ...
                            if (preg_match("!{$_ldl}{$al}\\\$smarty\.block\.child\s*{$_rdl}!", $_match2[7][$key])) {
                                ...
                                $block_content = preg_replace(
                                    "!(({$_ldl}{$al}block)(.*)?{$name}(.*)?({$_rdl}[\s\S]*?{$_ldl}{$al}/block\s*{$_rdl}))!",
                                    $replacement,
                                    $block_content
                                );
                            }
                            if (preg_match("!{$_ldl}{$al}\\\$smarty\.block\.child\s*{$_rdl}!", $_match2[8][$key])) {
                                ...
                                $block_content = preg_replace(
                                    "!(({$_ldl}{$al}block)(.*)?{$name}(.*)?({$_rdl})(.*)?({$_ldl}{$al}/block\s*{$_rdl}))!",
                                    $replacement,
                                    $block_content
                                );
                            }
                ...
                if (isset($template->block_data[$_name])) {
                    if (strpos($template->block_data[$_name]['source'], '%%%%SMARTY_PARENT%%%%') !== false) {
                        $template->block_data[$_name]['source'] = str_replace(
                            '%%%%SMARTY_PARENT%%%%',
                            $block_content,
