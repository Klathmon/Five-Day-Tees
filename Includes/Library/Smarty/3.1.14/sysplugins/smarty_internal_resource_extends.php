    ...
    public function getContent(Smarty_Template_Source $source)
    {
        ...
        foreach ($_components as $_component) {
            ...
            if ($_component != $_last) {
                ...
                preg_match_all(
                    "!{$_ldl}{$al}block\s(.+?)\s*{$_rdl}|{$_ldl}{$al}/block\s*{$_rdl}|{$_ldl}\*([\S\s]*?)\*{$_rdl}!",
                    $_content,
                    $_result,
                    PREG_OFFSET_CAPTURE
                );
                ...
                while ($_start + 1 < $_result_count) {
                    ...
                    if (($this->mbstring_overload ? mb_substr(
                            $_result[0][$_start][0],
                            0,
                            mb_strlen($source->smarty->left_delimiter, 'latin1') + 1,
                            'latin1'
                        ) : substr($_result[0][$_start][0], 0, strlen($source->smarty->left_delimiter) + 1)) == $source->smarty->left_delimiter . '*'
                    ) {
                        $_start++;
                        continue;
                    }
                    while ($_level != 0) {
                        ...
                        if (($this->mbstring_overload ? mb_substr(
                                ...
                                mb_strlen($source->smarty->left_delimiter, 'latin1') + 1,
                                'latin1'
                            ) : substr($_result[0][$_start + $_end][0], 0, strlen($source->smarty->left_delimiter) + 1)) ==
                            $source->smarty->left_delimiter . '*'
                        ) {
                            continue;
                        }
